...
def extract_listing_products(html: str, base_url: str) -> list[dict]:
    ...
    for item in soup.select('.product, .type-product, .product-item, .product-item-info'):
        ...
        name_el = item.select_one(
            '.woocommerce-loop-product__title, .product-item-link, '
            'h2 a, h3 a, h2, h3, [class*="product-name"], [class*="title"]'
        )
        name = name_el.get_text(strip=True) if name_el else (link_el.get('title', '') or link_el.get_text(strip=True))

        price_el = item.select_one(
            '.price .woocommerce-Price-amount, .price-box .price, '
            '.price ins .amount, .price .amount, .price, [class*="price"]'
        )
...
def extract_product_details(html: str) -> dict:
    ...
    if not price_text:
        price_el = soup.select_one(
            '.woocommerce-Price-amount, [itemprop="price"], '
            '.price-box .price, .price ins .amount, .summary .price'
        )
        if price_el:
    ...
    if availability == "Unknown":
        ...
        if avail_el:
            ...
            if 'InStock' in avail_text or 'in stock' in avail_text.lower():
                availability = "In Stock"
            elif 'OutOfStock' in avail_text or 'out of stock' in avail_text.lower():
                availability = "Out of Stock"
            elif 'PreOrder' in avail_text or 'pre-order' in avail_text.lower():
                availability = "Pre-Order"
...
def get_next_page_url(html: str, base_url: str) -> str | None:
    ...
    next_link = soup.select_one(
        '.woocommerce-pagination a.next, a.next.page-numbers, '
        '.pagination a.next, .pages a.next, a.action.next'
    )
    if next_link:
